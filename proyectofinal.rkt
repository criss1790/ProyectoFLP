...
'((white-sp
  ...
  (number("-" digit (arbno digit)) number))
  (float(digit (arbno digit)"."(arbno digit)float))
  (float("-"digit (arbno digit)"."(arbno digit)float))
  (identifier (letter (arbno (or letter digit))) symbol)
  )
...
(define grammar-simple-interpreter
  '((program(globals "PROGRAM" "{"expression "}") a-program)
    (globlas("GLOBALS" "{"(arbno global-declaration)"}") globals-block)
    (expression (number) lit-exp)
    (expression (float) lit-exp)
    ...
    (expression ("proc" "(" (separated-list optional-type-exp identifier ",") ")" expression)proc-exp)
